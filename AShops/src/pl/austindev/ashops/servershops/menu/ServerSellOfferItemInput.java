...
public class ServerSellOfferItemInput extends ServerShopInput {
	private ServerSellOfferItemInput(ServerShopSession session) {
		super(session);
	}

	public static ServerSellOfferItemInput open(ServerShopSession session,
			Player player) {
		ServerSellOfferItemInput menu = new ServerSellOfferItemInput(session);
	...
	public void onInput(final Player player, String message) {
		...
		if (item != null) {
			if (!getSession().getConfiguration().getForbiddenItems()
				...
				ServerSellOfferPriceInput.open(getSession(), player, item);
			} else {
