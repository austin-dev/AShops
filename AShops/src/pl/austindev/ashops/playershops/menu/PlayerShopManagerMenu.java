...
public class PlayerShopManagerMenu extends PlayerShopMenu {
	private final Inventory inventory;

	private PlayerShopManagerMenu(PlayerShopSession session) {
		super(session);
		this.inventory = Bukkit.createInventory(null, 9, getTitle());
		setupInventory();
	}

	public static PlayerShopManagerMenu open(PlayerShopSession session,
		Player player) {
		PlayerShopManagerMenu menu = new PlayerShopManagerMenu(session);
			...
			if (event.getRawSlot() <= event.getSlot()) {
				...
				case 0:
					if (event.isShiftClick()) {
						PlayerSellOfferItemInput.open(getSession(), player);
					} else {
						PlayerSellOfferItemSelectMenu
								.open(getSession(), player);
					}
					break;
				case 1:
					if (event.isShiftClick()) {
						PlayerBuyOfferItemInput.open(getSession(), player);
					} else {
						PlayerBuyOfferItemSelectMenu.open(getSession(), player);
					}
					break;
				case 2:
					handleShopRemove(player, event.isShiftClick());
					break;
				case 3:
	...
	private void handleShopRemove(final Player player, final boolean force) {
		getSession().end();
		player.closeInventory();
	...
	private void setupInventory() {
		setIcon(0, Material.BUCKET, ASMessageKey.MANAGER_ICON_SELL,
				ASMessageKey.MANAGER_ICON_SELL_CLICK,
				ASMessageKey.MANAGER_ICON_SELL_SHIFT);
		setIcon(1, Material.WATER_BUCKET, ASMessageKey.MANAGER_ICON_BUY,
		...
		setIcon(3, Material.CHEST, ASMessageKey.MANAGER_ICON_STATE,
