...
public class PlayerSellOfferItemInput extends PlayerShopInput {

	private PlayerSellOfferItemInput(PlayerShopSession session) {
		super(session);
	}

	public static PlayerSellOfferItemInput open(PlayerShopSession session,
			Player player) {
		PlayerSellOfferItemInput menu = new PlayerSellOfferItemInput(session);
	...
	public void onInput(final Player player, String message) {
		...
		if (item != null) {
			if (!getSession().getConfiguration().getForbiddenItems()
				...
				PlayerSellOfferPriceInput.open(getSession(), player, item);
			} else {
