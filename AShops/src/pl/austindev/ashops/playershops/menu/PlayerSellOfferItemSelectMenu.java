...
public class PlayerSellOfferItemSelectMenu extends PlayerShopMenu {
	...
	private PlayerSellOfferItemSelectMenu(PlayerShopSession session) {
		super(session);
		this.inventory = Bukkit.createInventory(null, INVENTORY_SIZE,
				getTitle());
	}

	public static PlayerSellOfferItemSelectMenu open(PlayerShopSession session,
			Player player) {
		PlayerSellOfferItemSelectMenu menu = new PlayerSellOfferItemSelectMenu(
				session);
		...
		if (item != null && !item.getType().equals(Material.AIR)) {
			if (event.getRawSlot() >= INVENTORY_SIZE) {
				if (!getSession().getConfiguration().getForbiddenItems()
					...
					PlayerSellOfferPriceInput.open(getSession(), player,
							new ItemStack(item));
				} else {
