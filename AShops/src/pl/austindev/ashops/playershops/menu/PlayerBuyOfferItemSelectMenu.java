...
public class PlayerBuyOfferItemSelectMenu extends PlayerShopMenu {
	private volatile Inventory inventory;
	private final int INVENTORY_SIZE = 9;

	private PlayerBuyOfferItemSelectMenu(PlayerShopSession session) {
		super(session);
		this.inventory = Bukkit.createInventory(null, INVENTORY_SIZE,
				getTitle());
	}

	public static PlayerBuyOfferItemSelectMenu open(PlayerShopSession session,
			Player player) {
		PlayerBuyOfferItemSelectMenu menu = new PlayerBuyOfferItemSelectMenu(
				session);
		...
		if (item != null && !item.getType().equals(Material.AIR)) {
			if (event.getRawSlot() >= INVENTORY_SIZE) {
				if (!getSession().getConfiguration().getForbiddenItems()
					...
					PlayerBuyOfferPriceInput.open(getSession(), new ItemStack(
							item), player);
				} else {
